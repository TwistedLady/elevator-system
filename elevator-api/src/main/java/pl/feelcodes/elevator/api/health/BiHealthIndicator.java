...
/**
 * The {@code bi} health component when BI is enabled: reports UP when the Spark BI fact table is
...
class BiHealthIndicator implements ReactiveHealthIndicator {
    ...
    BiHealthIndicator(@Value("${elevator.stats.fact-path:/data/elevator-facts.parquet}") String factPath) {
        this.parquetDir = Path.of(factPath);
    }
