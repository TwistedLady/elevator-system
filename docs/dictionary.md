...
| **passengerId** | Optional rider identity on a Call. The **authenticated username** (optional HTTP Basic on `POST /api/call`, see [auth](auth.md)), set by the api — never the request body; anonymous when no credentials. Lets the Manager count distinct riders vs. anonymous presses per order. |
