...
/// is anonymous (`None`); the high half maps to one of `PASSENGER_POOL` recurring riders
/// (`rider-N`), so roughly half the calls are identified and those riders repeat.
...
fn worker(
    ...
    for i in 0..n {
        ...
        if api::post_call_retry(&agent, api_base, elevator, floor, &id, rider.as_deref()).is_ok() {
